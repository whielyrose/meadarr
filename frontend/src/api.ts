...
export const api = {
  settings: {
    ...
    testSpotify:       ()     => post('/settings/test/spotify'),
  ...
  requests: {
    ...
    delete: (id: number)          => del(`/requests/${id}/delete`),
  ...
  playlists: {
    ...
    addTracks:      (id: number, data: AddTracksData) => post(`/playlists/${id}/tracks`, data),
...
export interface Release {
  mbid: string; title: string; artist: string; artist_mbid?: string
  year?: string; type?: string; score?: number
  in_library?: boolean; library_quality?: string; can_upgrade?: boolean
}
export interface Artist {
  mbid: string; name: string; sort_name?: string; type?: string
  country?: string; disambiguation?: string; score?: number
}
export interface Track {
  mbid?: string; title: string; track_number?: number
  disc_number?: number; duration_ms?: number
}
export interface ArtistRec {
  name: string; match?: number; similarity?: number; playcount?: number
  listen_count?: number; rank?: number; url?: string; image?: string
  reason?: string; in_library?: boolean
}
export interface AlbumRec {
  name: string; artist: string; playcount?: number; listen_count?: number
  rank?: number; url?: string; image?: string
  in_library?: boolean; library_quality?: string
}
export interface DownloadRequest {
  id: number; type: string; artist: string; album?: string; title?: string
  year?: number; status: string; format_pref: string; error_message?: string
  retry_count: number; requested_at: number; completed_at?: number
  task_count?: number; completed_tasks?: number; tasks?: DownloadTask[]
}
export interface DownloadTask {
  id: number; filename: string; peer: string; status: string
  expected_size?: number; downloaded_size?: number; dest_path?: string
}
export interface AlbumRequest {
  artist: string; album: string; year?: number; mbid?: string
  format_pref?: string; force?: boolean
}
export interface TrackRequest {
  artist: string; title: string; album?: string; mbid?: string; format_pref?: string
}
export interface RequestResult {
  status: string; request_id: number; message: string
}
export interface Playlist {
  id: number; name: string; description?: string; jellyfin_id?: string
  track_count?: number; auto_generated?: boolean
  created_at: number; updated_at: number; tracks?: PlaylistTrack[]
}
export interface PlaylistTrack {
  playlist_id: number; jellyfin_item_id?: string
  artist?: string; album?: string; title?: string; position: number
}
...
export interface LibraryStats {
  total_tracks: number; total_artists: number; total_albums: number
  formats: Record<string, number>; total_size_gb: number
}
export interface LibraryPage {
  total: number; page: number; per_page: number; tracks: LibraryTrack[]
}
export interface LibraryTrack {
  id: number; artist: string; album: string; title: string; year?: number
  track_number?: number; format?: string; bitrate?: number
  duration_ms?: number; file_path?: string; file_size?: number
}
export interface LibraryArtist {
  artist: string; album_count: number; track_count: number; formats: string
}
export interface LibraryAlbum {
  album: string; year?: number; track_count: number; format?: string
}
export interface ScanEntry {
  id: number; started_at: number; completed_at?: number
  files_found: number; files_added: number; files_updated: number
  files_removed: number; status: string
}
export interface TrackQueryParams {
  q?: string; artist?: string; album?: string; page?: number
}
